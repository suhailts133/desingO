...
1. Pick all relevant "space types" from this exact list: [${spaceTypes.join(", ")}].
   If nothing is mentioned or nothing is close, default to an array containing just ["${spaceTypes[0]}"].
2. Pick all relevant "design styles" from this exact list: [${designStyles.join(", ")}].
   If nothing is mentioned or nothing is close, default to an array containing just ["${designStyles[0]}"].
3. Write a vivid, detailed image-generation prompt (2-4 sentences) describing the space(s)
   in the selected style(s) — materials, color palette, lighting, key furniture/fixtures.
...
export async function structurePromptWithGroq(userPrompt: string): Promise<StructuredDesignPrompt> {
  ...
  if (!parsed.structuredPrompt || !parsed.matchedSpaceTypes || !parsed.matchedDesignStyles) {
