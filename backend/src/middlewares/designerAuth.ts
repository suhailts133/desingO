...
async function designerAuthentication(req: Request, res: Response, next: NextFunction) {
    try {
        ...
        if (decoded.role === USER_ROLES.DESIGNER) {
            ...
            return res.status(RESPONSE_CODE.UNAUTHORIZED).json({ message: AUTH_MESSAGES.AUTH.NOT_DESIGNER, success: false })
        }
        next()
    } catch (error) {
