...
async function customerAuthentication(req: Request, res: Response, next: NextFunction) {
    try {
        ...
        if (decoded.role === USER_ROLES.CUSTOMER) {
            ...
            return res.status(RESPONSE_CODE.UNAUTHORIZED).json({ message: AUTH_MESSAGES.AUTH.NOT_CUSTOMER, success: false })
        }
        next()
    } catch (error) {
