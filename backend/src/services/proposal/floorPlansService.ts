...
export class FloorPlansService implements IFloorPlanService {
    constructor(private _floorPlanRepo: IFloorPlanRepository, private _imageService: IImageUploaderService, private _proposalRepo: IProposalRepository) { }
    ...
    async acceptOrRejectFloorPlan(data: AcceptOrRejectFloorPlanDTO): Promise<IApiResponse> {
        ...
        if (updatedFloorPlan.status === VERSION_STATUS.APPROVED) {
            const updatedProposal = await this._proposalRepo.updateProposal(
                floorPlan.proposalId.toString(),
                { isFloorPlanApproved: true }
            );
            if (!updatedProposal) {
                throw new AppError(PROPOSAL_MESSAGES.PROPOSAL.UPDATE_FAILED, RESPONSE_CODE.INTERNAL_SERVER_ERROR);
            }

            await this._proposalRepo.openFirstServiceAndMarkOngoing(updatedProposal.sourceId.toString());
        }
