...
export class PaymentService implements IPaymentService {
  ...
  async markPaymentSucceeded(paymentIntentId: string, sourceId: string, order: number): Promise<void> {
    ...
    await this._tranasctionManager.runInTransaction(async (session) => {
      const updatePaymentStatus = await this._paymentRepo.updateStatus(paymentIntentId, Payment_Status.SUCCEEDED, session);
      ...
      const serviceStatusChanged = await this._proposalRepo.updateService(sourceId, order, ServiceStatus.IN_PROGRESS, ServicePaymentStatus.PAID, escrowData, servicePlatformFee, designerPayout, session);
      if (!serviceStatusChanged) {
        throw new AppError(PROPOSAL_MESSAGES.SERVICE.STATUS_NOT_CHANGED, RESPONSE_CODE.INTERNAL_SERVER_ERROR);
      }
    });
  }
  ...
  async markPaymentFailed(paymentIntentId: string): Promise<void> {
    ...
    const serviceStatusChanged = await this._proposalRepo.updateService(updated.jobId.toString(), updated.serviceOrder, ServiceStatus.OPEN, ServicePaymentStatus.PENDING, {});
    if (!serviceStatusChanged) {
      throw new AppError(PROPOSAL_MESSAGES.SERVICE.STATUS_NOT_CHANGED, RESPONSE_CODE.INTERNAL_SERVER_ERROR);
    }
  }
