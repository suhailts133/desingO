...
export class ProposalService implements IProposalService {
  constructor(
    ...
    private _activeRepo: IActiveJobRepository,
  ...
  async createProposal(data: CreateProposalDTO): Promise<IApiResponse> {
    ...
    await this._transactionManager.runInTransaction(async (session) => {
      console.log(session.id, "create proposal");
      const result = await this._proposalRepo.createProposal(repoData, session);
      if (!result) {
        throw new AppError(PROPOSAL_MESSAGES.PROPOSAL.CREATION_FAILED, RESPONSE_CODE.INTERNAL_SERVER_ERROR);
      }

      const activeJobProposalStatus = await this._activeRepo.updateActiveJob(data.sourceId, { proposalStatus: "CREATED" }, session);
      if (!activeJobProposalStatus) {
        throw new AppError(JOB_MESSAGES.ACTIVE_JOB.PROPOSAL_STATUS_FAILED, RESPONSE_CODE.INTERNAL_SERVER_ERROR);
      }
    });
