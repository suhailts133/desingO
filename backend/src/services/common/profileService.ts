...
export class ProfileService implements IProfileService {
  constructor(
    private _DesignerRepo: IDesignerRepository,
  ...
  async updateDesignerProfile(designerId: string, data: DesignerUpdateResponseDTO): Promise<IApiResponse<DesignerUpdateResponseDTO>> {
    ...
    const { updatedDesigner, finalUser } = await this._transactionManager.runInTransaction(async (session) => {
      const updatedDesigner = await this._DesignerRepo.updateDesigner(designerId, designerData,session);
      if (!updatedDesigner) {
        throw new AppError(PROFILE_MESSAGES.PROFILE.UPDATE_FAIL, RESPONSE_CODE.INTERNAL_SERVER_ERROR);
      }
      ...
      if (full_name !== currentUser.full_name) {
        const updatedUser = await this._userRepo.updateUser(designerId, { full_name },session);
        if (!updatedUser) {
          throw new AppError(PROFILE_MESSAGES.PROFILE.UPDATE_FAIL, RESPONSE_CODE.INTERNAL_SERVER_ERROR);
        }
        finalUser = updatedUser;
      }
      return { updatedDesigner, finalUser };
    });
