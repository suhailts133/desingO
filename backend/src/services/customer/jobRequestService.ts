...
export class JobRequestService implements IJobRequestService {
  constructor(
    ...
    private _designerInteractionRepo: IDesignerInteractionRepository,
  ...
  async editJobRequest(jobId: string, data: EditJobRequest, refrenceImages?: Express.Multer.File[], floorPlanImages?: Express.Multer.File[]): Promise<IApiResponse> {
    ...
    const imagesToDeleteReference = jobRequest.referenceImages.filter((img) => !oldReferencesFiles.has(img.filename)).map((img) => img.filename);

    const imagesToDeleteFloorPlans = jobRequest.floorPlans.filter((img) => !oldfloorPlanFiles.has(img.filename)).map((img) => img.filename);
    ...
    const finalRefrenceImages: ImageUploadResult[] = [...(oldReferences ?? []), ...(newRefrenceImages ?? [])];
    const finalFloorPlans: ImageUploadResult[] = [...(oldFloorPlans ?? []), ...(newFloorPlans ?? [])];
  ...
  async getJobRequestDetail(jobId: string, designerId?: string): Promise<IApiResponse<JobDetailResponseDTO>> {
    ...
    if (designerId) {
      await this._designerInteractionRepo.createInteraction({
        designerId,
        jobId,
        weight: JOB_INTERACTION.VIEW,
        action: JOB_INTERACTION_TYPE.VIEW,
      });
    }
