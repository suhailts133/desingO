...
export class AdminDesignerVerificationservice implements IAdminDesignerVerificatoinServices {
    ...
    async getallDesignerRequests(filter?: DesignerFilterDTO): Promise<IApiResponseWithPagination<AdminDesignersResponseDTO[]>> {
        ...
        return { message: MESSAGES.ADMIN_DESIGNER_VERFICATION.DESIGNER_APPLICATIONS, data: data, total: pagination.total, totalPages: pagination.totalPages }
    ...
    async getDesignerRequest(id: string): Promise<IApiResponse<AdminDesignerRequestResponseDTO>> {
        ...
        if (!data) {
            throw new AppError(MESSAGES.ADMIN_DESIGNER_VERFICATION.DESIGNER_APPLICATION_NOT_FOUND, RESPONSE_CODE.NOT_FOUND)
        }
        return { message: MESSAGES.ADMIN_DESIGNER_VERFICATION.DESIGNER_APPLICATION_DETAIL, data };
    ...
    async ApproveOrRejectDesignerRequest(id: string, data: AdminDesignerApprovalDTO): Promise<IApiResponse<AdminDesignerStatusDTO>> {
        ...
        if (!result) {
            throw new AppError(MESSAGES.ADMIN_DESIGNER_VERFICATION.STATUS_CHAGNE_NOT_FOUND, RESPONSE_CODE.INTERNAL_SERVER_ERROR)
        ...
        if (result.status === DESIGNER_STATUS.APPROVED) {
            ...
            if (!statusChanged) {
                throw new AppError(MESSAGES.ADMIN_DESIGNER_VERFICATION.STATUS_CHANGE_USER_NOT_FOUND, RESPONSE_CODE.INTERNAL_SERVER_ERROR)

            }
            if (statusChanged.role !== USER_ROLES.DESIGNER) {
                throw new AppError(MESSAGES.ADMIN_DESIGNER_VERFICATION.STATUS_CHANGE_FAIL, RESPONSE_CODE.INTERNAL_SERVER_ERROR)
            }
            await sendDesignerStatusEmail(result.email, result.name, result.status)
            return { message: MESSAGES.ADMIN_DESIGNER_VERFICATION.STATUS_CHANGE_SUCCESS, data: { status: result.status } };
        } else {
