...
export class ProposalRepository extends BaseRepository<IProposal> implements IProposalRepository {
  ...
  async updateService(sourceId: string, order: number, status: ProposalServiceStatus, paymentStatus: PaymentUpdateStatus, escrow: Partial<IEscrow>, feeDeduction?: number, currentAmountHeld?: number, session?: ClientSession): Promise<IProposal | null> {
    const updateDoc: UpdateQuery<IProposal> = {
      $set: {
        "services.$.status": status,
  ...
  async getProposal(sourceId: string): Promise<GetProposalDTO | null> {
    return await this._model.findOne({ sourceId }).populate<{ clientId: IUser }>("clientId").populate<{ designerId: IUser }>("designerId");
  }
