...
export class PaymentRepository extends BaseRepository<IPayment> implements IPaymentRepository {
  ...
  async updateStatus(stripePaymentIntentId: string, status: PaymentStatus, session?: ClientSession): Promise<IPayment | null> {
    console.log(session?.id, "payment Repo update Status");
    return this.updateOne({ stripePaymentIntentId }, { $set: { status } }, session);
  }
}
